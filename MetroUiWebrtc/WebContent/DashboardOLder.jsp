...
<body >
	...
	<%System.out.println("in dashboard getPrivateIdentity "+login1.getPrivateIdentity().toString()); %>
...
<script type="text/javascript">
	 ...
	 function responseFromServletForAddFrd(){
		 ...
		 alert("IN RESPONSE FROM SERVLET For Add Frd");
		if(xmlHttpRequest2.readyState==4&&xmlHttpRequest2.status==200)
		{
			...
			$.each(jsonResponseFromServer.Friends, function(key, val) {
				...
				/* $.each(jsonResponseFromServer.presenceForFrnd, function(key, val) {
					alert(key);
					$.each(val, function(k, v){
						if("offline".match(v)){
							$('<td><img src="images/offline.gif" style="width: 10px;height: 10px;"</td>').appendTo(tr);
...
<div id="friendList" class="reveal-modal" >
	...
	<div style="height: 45%" id="frndDiv">
		...
		<div style="height: 30%;">
		<button style="width: 50px; margin-right: 5px;" id="call">Call</button>
		<button style="width: 60px; margin-right: 5px;" id="message">Message</button>
		<button style="width: 50px; margin-right: 5px;"id="view" onclick="getFriend();">View</button>
		<button style="width: 50px; margin-right: 5px;" id="add" onclick="addToFriendList();">Add </button>
		</div>
		<div style="height: 65%;">
		
		<%-- <%if(!"".equalsIgnoreCase(login2.getDisplayName())&&null!=login2.getDisplayName()){
		...
		<tr>
    	         ...
    	         <td>
					...
					<input type="text"
						id="displayName1" name="displayName1"  class="round full-width-input" readonly="readonly"/>
        	     </td>		
		</tr>
		<tr>
			<td>
			<label for="login-username">PublicIdentity</label>
			</td>
			<td>
			 <input type="text" id="publicIdentity1" name="publicIdentity1"  class="round full-width-input" readonly="readonly" />
			</td>
		</tr>
		<tr>
		<td>
		<label for="login-username">PrivateIdentity</label>
		</td>
		<td>
		
		<input type="text" id="privateIdentity1" name="privateIdentity1"  class="round full-width-input" readonly="readonly"/>
		</td>
		</tr>
		
		<tr>
		<td>
		<label for="login-username">Realm</label> 
		</td>
		<td>
		<input type="text" id="realm1" name="realm1" 
						class="round full-width-input" readonly="readonly"/>
		</td>
		</tr>		
		<tr><td>
		Profile Pic:</td><td><img id="friendImage"  style="width:100px;height: 80px;"></td>
	...
	<div id="geoLocationModal" class="reveal-modal xlarge">
		...
		<table align="center" style="padding-top:15px;">
			<tr>
			<td align="center" style="padding-right:15px;"><input type="button" id="shareGeoLoc" onclick="shareMyLocation();" value="Share my location"/></td>
			...
			<!-- <td><a onclick="findGeoLocFrd();"><font style="font-size: 15px;">View Location of Friends</font> </a></td> --></tr>
		</table> 
...
<td><input type="submit" name="submit" value="Send_to_gmail">
<img src="socialmediaimages/1382540053_Android-Gmail.png" width="40" height="40"></td>
</tr>
<tr>
<td style="float:left; color: black; font-size: 15px;">Send Message via Facebook :</td> 
<td><input type="submit" name="submit" value="Send_to_facebook">
<img src="socialmediaimages/1382539943_facebook_02.png" width="40" height="40"></td>
</tr>
<tr>
<td style="float:left; color: black; font-size: 15px;">Send Message via Yahoo :</td> 
<td><input type="submit" name="submit" value="Send_to_yahoo">
<img src="socialmediaimages/1382540139_picons14.png" width="40" height="40"></td>
</tr>
				...
				<section id="content-mos" class="centered clearfix">
					...
					<a href="#" data-reveal-id="userProfile" data-animation="fade">
						<div class="tile large live" data-stops="0" data-speed="3000"
							...
							<%-- <%=login1.getPublicIdentity().toString()%> --%>
